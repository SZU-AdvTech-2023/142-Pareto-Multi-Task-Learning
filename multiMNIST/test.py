...
def test(dataset, base_model):
    ...
    if dataset == 'mnist':
        ...
        if base_model == 'lenet':
            with open('saved_model/mnist_lenet_niter_100_npref_5_prefidx_4.pickle', 'rb') as f:
                state_dict = torch.load(f)
                model = RegressionTrain(RegressionModel(n_tasks), init_weight)
                model.model.load_state_dict(state_dict)
        if base_model == 'resnet18':
            with open('saved_model/mnist_resnet18_niter_20_npref_5_prefidx_4.pickle', 'rb') as f:
                state_dict = torch.load(f)
    ...
    if dataset == 'fashion':
        with open('data/multi_fashion.pickle', 'rb') as f:
            trainX, trainLabel, testX, testLabel = pickle.load(f)
        if base_model == 'lenet':
            with open('saved_model/fashion_lenet_niter_100_npref_5_prefidx_4.pickle', 'rb') as f:
                state_dict = torch.load(f)
                model = RegressionTrain(RegressionModel(n_tasks), init_weight)
                model.model.load_state_dict(state_dict)
        if base_model == 'resnet18':
            with open('saved_model/fashion_resnet18_niter_20_npref_5_prefidx_4.pickle', 'rb') as f:
                state_dict = torch.load(f)
                model = RegressionTrainResNet(MnistResNet(n_tasks), init_weight)
                model.model.load_state_dict(state_dict)
    ...
    if dataset == 'fashion_and_mnist':
        with open('data/multi_fashion_and_mnist.pickle', 'rb') as f:
            trainX, trainLabel, testX, testLabel = pickle.load(f)
        if base_model == 'lenet':
            with open('saved_model/fashion_and_mnist_lenet_niter_100_npref_5_prefidx_4.pickle', 'rb') as f:
                state_dict = torch.load(f)
                model = RegressionTrain(RegressionModel(n_tasks), init_weight)
                model.model.load_state_dict(state_dict)
        if base_model == 'resnet18':
            with open('saved_model/fashion_and_mnist_resnet18_niter_20_npref_5_prefidx_4.pickle', 'rb') as f:
                state_dict = torch.load(f)
                model = RegressionTrainResNet(MnistResNet(n_tasks), init_weight)
                model.model.load_state_dict(state_dict)
